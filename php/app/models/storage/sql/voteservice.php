<?php
    ...
    class VoteService extends \Core\Model {
        ...
        public function getPopular($max){
            $max = intval($max);
            $q = $this->db->prepare("SELECT ph_posts.id FROM ph_posts ORDER BY 3600*24*2*(IFNULL((SELECT ABS(SUM(ph_votes.vote)) FROM ph_votes WHERE ph_votes.post = ph_posts.id),0) + (SELECT COUNT(id) as comments FROM ph_comments WHERE ph_comments.post = ph_posts.id) + 1)/(?-ph_posts.date) DESC LIMIT ".$max);
